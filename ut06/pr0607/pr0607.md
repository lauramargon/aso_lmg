...
```bash
$inputPath = "$PSScriptRoot/nuevos_empleados_raw.txt"
$outputPath = "$PSScriptRoot/usuarios_importar.csv"

$textInfo = (Get-Culture).TextInfo

$empleadosProcesados = Get-Content -Path $inputPath | ForEach-Object {
    if ([string]::IsNullOrWhiteSpace($_)) { return }

    $campos = $_ -split '\|'
    $nombreSucio = $campos[0]
    $codigo      = $campos[1]
    $fecha       = $campos[2] 


    $partesNombre = $nombreSucio -split ','
    $apellidos = $partesNombre[0].Trim()
    $soloNombre = $partesNombre[1].Trim()


    $nombreCompleto = $textInfo.ToTitleCase(($soloNombre + " " + $apellidos).ToLower())


    $primerApellido = ($apellidos -split ' ')[0].ToLower()
    $primeraLetra = $soloNombre.Substring(0,1).ToLower()
    

    $letrasApellido = if ($primerApellido.Length -gt 6) { $primerApellido.Substring(0,6) } else { $primerApellido }
    $usuario = $primeraLetra + $letrasApellido


    $emailLimpio = ($soloNombre + "." + $apellidos).Replace(" ", ".").Replace("'", "").ToLower()
    $email = $emailLimpio + "@techiberia.com"

    $departamento = ($codigo -split '-')[1].Replace("_", " ")

    $anioNacimiento = ($fecha -split '/')[0]
...
$empleadosProcesados | Export-Csv -Path $outputPath -NoTypeInformation -Encoding UTF8
$empleadosProcesados | Format-Table -AutoSize
