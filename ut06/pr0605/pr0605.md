...
$resultado = foreach ($line in $lineas) {
...
") {
    ...
    if ($line -match "ID:\d+\s*::\s*(.*?)\s*::") {
        $fechaCruda = $matches[1]


        $fechaNormalizada = $fechaCruda -replace "/", "-" -replace "_", " "


        $timestamp = Get-Date $fechaNormalizada -Format "yyyy-MM-dd HH:mm"
    }


    if ($line -match "User:(\S+)") {
        $user = $matches[1].ToLower()
